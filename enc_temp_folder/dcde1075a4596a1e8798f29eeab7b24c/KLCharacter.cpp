...
AKLCharacter::AKLCharacter()
{
	...
	TraceComp = CreateDefaultSubobject<UKLInteractingTraceComponent>("TraceComp"); 
	TraceComp->CameraComp = Camera; 
...
void AKLCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Interact", IE_Pressed, this, &AKLCharacter::Interact); 
}
...
void AKLCharacter::ReleaseBow()
{
	...
	if(EquippedBow != nullptr && bIsAiming)
	{
		...
		AimRotation.Yaw += HorizontalAimOffset; 
		AimRotation.Pitch += VerticalAimOffset; 

		EquippedBow->ReleaseBow(AimRotation);
...
void AKLCharacter::Interact()
{
	TraceComp->InteractWith(); 
